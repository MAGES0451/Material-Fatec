...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($arquivoNovo['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($extension, array('jpg', 'png', 'jpeg', 'gif'))) {
            ...
            $stmt = $conn->prepare("UPDATE produtos SET nome = ?, descricao = ?, categoria = ?, preco = ?, quantidade = ?, imagem = ? WHERE id = ?");
            $stmt->execute([$nome, $descricao, $categoria, $preco, $quantidade, $nomeArquivoNovo, $id]);
        ...
        $stmt = $conn->prepare("UPDATE produtos SET nome = ?, descricao = ?, categoria = ?, preco = ?, quantidade = ? WHERE id = ?");
        $stmt->execute([$nome, $descricao, $categoria, $preco, $quantidade, $id]);
        header("Location: lista_produtos.php");
        exit;
    }
}
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="categoria" id="categoria" required><?= htmlspecialchars($produto['categoria']) ?></textarea><br>
                    </div>
                    <div class="form-group">
